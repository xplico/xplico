...
<div class="generic">
<h2><?php echo __('Webmail to'); ?> <?php echo str_replace('>', '&gt;', str_replace('<', '&lt;', $mailObj['to']))?></h2>
...
<td class="date"><?php echo $this->Html->link(__('email.eml'), '/webmails/eml') ?></td>
</tr>
<tr>
<td class="header-title"><?php echo __('Info:'); ?></td>
<td class="date pinfo"><a href="#" onclick="popupVetrina('/webmails/info','scrollbar=auto'); return false"><?php echo __('info.xml'); ?></a><div class="ipcap"><?php echo $this->Html->link('pcap', 'pcap/'); ?></div></td>
</tr>
</tbody></table>
...
<?php if (isset($mailObj['Attachments'])) : ?>
  ...
  <?php foreach($mailObj['Attachments'] as $attachment) : ?>
    ...
    <td class="date"><?php echo $this->Html->link($attachment['FileName'], '/webmails/content'.strrchr($attachment['DataFile'], '/')) ?></td>
    <?php elseif (isset($attachment['Description'])) : ?>
    <td class="date"><?php echo $this->Html->link($attachment['Description'], '/webmails/content'.strrchr($attachment['DataFile'], '/')) ?></td>
    <?php endif; ?>
    </tr>
  <?php endforeach; ?>
  </tbody></table>
