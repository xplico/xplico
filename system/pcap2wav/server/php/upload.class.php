...
class UploadHandler
{
    ...
    function __construct($options=null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']).'/files/'.$ip.'/',
            'upload_url' => $this->getFullUrl().'/files/'.$ip.'/',
            'param_name' => 'files',
        ...
        mkdir($this->options['upload_dir'].'/pcap');
        if ($options) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                copy($file_path, $this->options['upload_dir'].'/pcap/'.time().'_'.$file->name);
    ...
    public function getwav() {
        $info = $this->get_file_objects('.wav');
        header('Content-type: application/json');
        echo json_encode($info);
    }
