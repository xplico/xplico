...
class PolsController extends AppController {
    ...
    function add() {
        ...
        if (!empty($this->data)) {
            ...
            if ($register['Param']['nvalue'] == 0)
                $this->data['Pol']['realtime'] = $this->data['Capture']['Type'];
            else
                $this->data['Pol']['realtime'] = 0;
            $this->data['Pol']['group_id'] = $this->Session->read('group');
            if($this->Pol->save(Sanitize::paranoid($this->data))) {
                $this->Session->setFlash(__('The Case has been created', true));
                $this->redirect('/pols/index');
            } else {
    ...
    function delete($id = null) {
        ...
        if ($pol['Pol']['group_id'] != $this->Session->read('group') || $this->Session->check('admin')) {
            if ($this->Session->check('admin')) {
                $this->Session->setFlash(__('Admin can not delete the cases', true));
            }
            else {
                $this->Session->setFlash(__('Invalid id for the Case', true));
            }
            $this->redirect('/pols/index');
        }
