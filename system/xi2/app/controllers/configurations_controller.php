...
class ConfigurationsController extends AppController
{
 ...
 function beforeFilter() {
    ...
    function index() {
	if (isset($this->data['Param']['startXplico']) ) {
            //A way to start/restart Xplico from the web interface (aka "xi")
            $this->Xplico->startStopXplico($this->data['Param']['startXplico']);
        }
    ...
    function geoposition() {
        if (!empty($this->data)) {
            if (is_numeric($this->data['GPSposition']['long']) == FALSE ||
                is_numeric($this->data['GPSposition']['lat']) == FALSE)
            ...
            foreach ($files as $cfg) {
                ...
                foreach ($cfgparams as $line) {
                    if (strstr($line, 'DISPATCH_GEPMAP_LAT=') != FALSE) {
                        fwrite($fh, 'DISPATCH_GEPMAP_LAT='.$this->data['GPSposition']['lat']."\n");
                    }
                    else if (strstr($line, 'DISPATCH_GEPMAP_LONG=') != FALSE) {
                        fwrite($fh, 'DISPATCH_GEPMAP_LONG='.$this->data['GPSposition']['long']."\n");
                    }
                    else {
                        fwrite($fh, $line);
                    }
