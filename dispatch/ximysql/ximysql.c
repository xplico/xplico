/* ximysql.c
 *
 * Xplico System dispatcher module for MySQL
 *
 *
...
static int ReadConfigFile(const char *path, dbconf *db_c);
...
static int DispHostDb(const char *ip, int pol, int sol)
{
    ...
    ret = mysql_query(conn, query_l);
    if (ret == 0) {
        ret = -1;
        result = mysql_store_result(conn);
        if (result != NULL && result->row_count != 0) { /* there are rows */
                ...
                if (title != NULL) {
                    ...
                    if (end != NULL) {
                        ...
                        if (res == 0) {
                            res = -1;
                            result = mysql_store_result(conn);
                            if (result && result->row_count != 0) { /* there are rows */
                                row = mysql_fetch_row(result);
                                id = atol(row[0]);
                                mysql_free_result(result);
...
static int DispMms(pei *ppei)
{
    ...
    /* flow info */
    sprintf(flow_info, XS_MMS_DIR_PATH"/flow_%lu_%i.xml", pol, sess, time(NULL), inc++);
    DispFlowInfo(flow_info, ppei->stack);
    /* compose query and insert record */
...
static int DispDns(pei *ppei)
{
    ...
    if (ip_one != NULL || cname != NULL) {
        ...
        /* flow info */
        sprintf(flow_info, XS_DNS_DIR_PATH"/flow_%lu_%i.xml", pol, sess, time(NULL), inc++);
        DispFlowInfo(flow_info, ppei->stack);
...
static int DispArp(pei *ppei)
{
    ...
    if (ip != NULL && mac != NULL) {
        /* flow info */
        sprintf(flow_info, XS_ARP_DIR_PATH"/flow_%lu_%i.xml", pol, sess, time(NULL), inc++);
        DispFlowInfo(flow_info, ppei->stack);
...
static int DispIcmpv6(pei *ppei)
{
    ...
    if (ip != NULL && mac != NULL) {
        /* flow info */
        sprintf(flow_info, XS_ICMPv6_DIR_PATH"/flow_%lu_%i.xml", pol, sess, time(NULL), inc++);
        DispFlowInfo(flow_info, ppei->stack);
...
static int DispPaltalkExp(pei *ppei)
{
    ...
    while (cmpn != NULL) {
        if (cmpn->eid == pei_pltk_e_user_id) {
            ...
            duration = cmpn->strbuf;
            duration = duration; // to avoid warning
        }
...
static int DBInit(dbconf *db_c)
{
    ...
    ret = mysql_real_connect(
        ...
        db_c->host,     /* host to connect to */
        db_c->user,     /* user name */
        db_c->password, /* password */
        db_c->name,     /* database to use */
        0,              /* port (use default) */
...
{
    LogPrintf(LV_DEBUG, "MySQL Dispatcher");
...
static int CfgParIsComment(char *line)
{
    char *cmnt;
...
static int ReadConfigFile(const char *path, dbconf *db_c)
{
    FILE *fp;
    int res, nl;
    char buffer[XS_STR_DIM];
    char bufcpy[XS_STR_DIM];
    char *param;
    ...
    nl = 0;
    memset(db_c, '\0', sizeof(dbconf));
    while (fgets(buffer, XS_STR_DIM, fp) != NULL) {
        nl++;
        /* check all line */
        if (strlen(buffer)+1 == XS_STR_DIM) {
            printf("Config file line more length to %d characters", XS_STR_DIM);
            return -1;
        }
        ...
        if (!CfgParIsComment(buffer)) {
            ...
            if (param != NULL) {
                if (db_c->host[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_HOST);
                    return -1;
                }
            ...
            if (param != NULL) {
                if (db_c->name[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_NAME);
                    return -1;
                }
            ...
            if (param != NULL) {
                if (db_c->user[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_USER);
                    return -1;
                }
            ...
            if (param != NULL) {
                if (db_c->password[0] != '\0') {
                    printf("Config param error: param '%s' defined two times", CFG_PAR_DB_PASSWORD);
                    return -1;
                }
